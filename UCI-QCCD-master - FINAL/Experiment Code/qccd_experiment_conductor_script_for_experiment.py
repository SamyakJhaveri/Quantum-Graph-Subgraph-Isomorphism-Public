...
# ---------- x ---------- x ---------- x ---------- 
    ...
    """
    input_file_name_smaller = "programs_sample1_2.csv"
    input_file_path_smaller = os.path.join(root_path, input_folder_path)
    input_file_path_smaller = os.path.join(input_folder_path, "two-sample programs/" + input_file_name_smaller)
    print("input_file_path:{}".format(input_file_path_smaller))
    """
    ...
    """
    output_file_name_energy_smaller = "results_sample1_2_energy.csv"
    output_file_path_energy_smaller = os.path.join(output_folder_path, "two-sample results/" + output_file_name_energy_smaller)
    print("output_file_path_energy_smaller:{}".format(output_file_path_energy_smaller))

    output_file_name_timing_smaller = "results_sample1_2_timing.csv"
    output_file_path_timing_smaller = os.path.join(output_folder_path, "two-sample results/" + output_file_name_timing_smaller)
    print("output_file_path_timing_smaller:{}".format(output_file_path_timing_smaller))
    
    output_file_name_minnode_smaller = "results_sample1_2_minnode.csv"
    output_file_path_minnode_smaller = os.path.join(output_folder_path, "two-sample results/" + output_file_name_minnode_smaller)
    print("output_file_path_minnode_smaller:{}".format(output_file_path_minnode_smaller))

    output_file_name_mapping_smaller = "results_sample1_2_mapping.csv"
    output_file_path_mapping_smaller = os.path.join(output_folder_path, "two-sample results/" +  output_file_name_mapping_smaller)
    print("output_file_path_mapping_smaller:{}".format(output_file_path_mapping_smaller))

    # output_graph_folder_path_smaller = os.path.join(output_folder_path, "two-sample results/" + "graph_sample1_2_smaller")     
    # print("output_graph_folder_path_smaller:{}".format(output_graph_folder_path_smaller))

    # if not os.path.exists(output_graph_folder_path_smaller):
    #     print("making {}".format(output_graph_folder_path_smaller))
    #     os.makedirs(output_graph_folder_path_smaller)
    """
    ...
    with open(output_file_path_energy, 'w', newline = '') as output_file_energy,\
        ...
        for i in range(len(ncs)): # i is row number
            ...
            for j in range(i + 1): # j is column number
                ...
                """
                inv_node_mapping = {v: k for k, v in updated_best_mapping.items()}
                # relabel the nodes in the target graph with the node labels from the graph to embed
                updated_nodes = nx.relabel_nodes(resG1, inv_node_mapping, copy = True)
                
                
                pos = nx.spring_layout(updated_nodes, seed = 1000)
                print("updated_nodes:{}".format(updated_nodes))
                # updated_best_mapping = {str(k).split()[0][1:]: str(G1_nodes[v]).split()[0][1:] for k, v in best_mapping.items()}
                # Create a dictionary that maps each node to its label
                ...
                nx.draw(updated_nodes, 
                        ...
                        with_labels = True, 
                        font_color = 'red',
                        nodelist = {key: str(key).split()[0][1:] for key in updated_best_mapping.keys()},
                        node_color = "tab:red")
                ...
                nx.draw(updated_nodes, 
                        ...
                        with_labels = True, 
                        font_color = 'black', 
                        nodelist = list(set(updated_nodes.nodes()) - set(updated_best_mapping.keys())), # node2_type = str(node2).split()[0][1:]
                        node_color = "tab:blue")
                # plt.show()
                plt.savefig(output_graph_folder_path_smaller+"/graph_" + ncs[i][0] + "(" + str(NXGraph_G1.number_of_nodes()) + ")_"+ ncs[j][0] + "(" + str(NXGraph_G2.number_of_nodes()) + ")" + ".png")
                """
                time.sleep(5)
                count += 1
            print("--x--")
